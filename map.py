...
class MapParser:
    ...
    def __lines(
        ...
        Strips comments and ignores empty lines. Yields (-1, "") at the end
        of the file.

        Yields:
            Tuple[int, str]: A tuple containing the line number and the line
                content.
        """
    ...
    def parse(self) -> MapAttributes:
        ...
        Returns:
            MapAttributes: A tuple containing the number of drones, the
                start hub,
                a dictionary of normal hubs, and the end hub.

        Raises:
            MapFormatError: If there's a syntax or formatting error in the
                map file.
            MapLogicError: If there's a logical error in the map definition
                (e.g. duplicate names).
        """
        ...
        if n == -1:
            raise MapFormatError(
                ErrorInfo(
                    ...
                    error_end=0,
                    reason=(
                        "the file is empty or has only a commend and empty "
                        "lines"
                    ),
        ...
        if not match:
            raise MapFormatError(NbDronesLineInspector.inspect(line, n))
        ...
        if key.lower() != "nb_drones":
            raise MapFormatError(
                ErrorInfo(
                    ...
                    reason=(f"'{key}' is not" " the expected key"),
                    expected=("nb_drones as the first line key"),
                    how_to_fix=("rename the key to nb_drones"),
                )
            )
        ...
        try:
            ...
            raise MapFormatError(
                ErrorInfo(
                    ...
                    reason=(f"'{value_str}' is not" " a valid integer"),
                    expected=("a positive integer" " for drone count"),
                    how_to_fix=(
                        "replace the value with" " a positive integer"
                    ),
                )
            )
        ...
        if nb_drones <= 0:
            raise MapFormatError(
                ErrorInfo(
                    ...
                    error_start=match.start("value"),
                    error_end=match.end("value"),
                    reason=(f"'{nb_drones}' is not" " a positive value"),
                    expected=("a positive integer" " greater than 0"),
                    how_to_fix=("use a positive number" " for drone count"),
                )
            )
                ...
                if hub.name in all_hubs:
                    raise MapLogicError(
                        ErrorInfo(
                            ...
                            error_start=0,
                            error_end=len(line),
                            reason=(f"hub '{hub.name}'" " is already defined"),
                            expected=("each hub to have" " a unique name"),
                            how_to_fix=(
                    ...
                    if start_hub is not None:
                        raise MapLogicError(
                            ErrorInfo(
                                ...
                                error_start=0,
                                error_end=len(line),
                                reason=("a start hub is" " already defined"),
                                expected=("only one start hub" " in the map"),
                                how_to_fix=(
                                    "remove this duplicate" " start hub"
                                ),
                            )
                        )
                    start_hub = hub
                    ...
                    if end_hub is not None:
                        raise MapLogicError(
                            ErrorInfo(
                                ...
                                error_start=0,
                                error_end=len(line),
                                reason=("an end hub is" " already defined"),
                                expected=("only one end hub" " in the map"),
                                how_to_fix=(
                                    "remove this duplicate" " end hub"
                                ),
                            )
                        )
                    end_hub = hub
        ...
        if not start_hub:
            raise MapLogicError(
                ErrorInfo(
                    ...
                    error_start=0,
                    error_end=0,
                    reason=("no start hub is" " defined in the map"),
                    expected="exactly one start_hub line",
                    how_to_fix=("add a start_hub line to the map"),
                )
            )
        ...
        if not end_hub:
            raise MapLogicError(
                ErrorInfo(
                    ...
                    error_start=0,
                    error_end=0,
                    reason=("no end hub is" " defined in the map"),
                    expected="exactly one end_hub line",
                    how_to_fix=("add an end_hub line to the map"),
                )
            )
...
class Map:
    ...
    Contains the start hub, end hub, intermediate hubs, and the fleet of
    drones.
    """
    ...
    def hubs(self) -> Dict[str, Hub]:
        """Dict[str, Hub]: A dictionary of intermediate and priority hubs
